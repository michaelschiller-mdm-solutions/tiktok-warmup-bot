...
CREATE TABLE IF NOT EXISTS iphones (
  ...
  ssh_user VARCHAR(255) DEFAULT 'mobile',
...
CREATE TABLE IF NOT EXISTS iphone_containers (
  ...
  -- Container metadata
  container_uuid UUID DEFAULT gen_random_uuid(),
...
COMMENT ON COLUMN iphones.name IS 'Unique user-defined name for the iPhone';
COMMENT ON COLUMN iphones.ip_address IS 'IP address for connecting to the iPhone';
COMMENT ON COLUMN iphones.ssh_password_encrypted IS 'Encrypted SSH password';
COMMENT ON COLUMN iphones.assigned_bot_id IS 'ID of the bot assigned to this iPhone';
