...
// Add new endpoints for warmup configuration

/**
 * GET /api/models/:id/warmup-config
 * Get warmup configuration for a model
 */
router.get('/:id/warmup-config', async (req: any, res: any) => {
  try {
    ...
    if (result.rows.length === 0) {
      ...
      // Fetch the newly created configuration
...
/**
 * PUT /api/models/:id/warmup-config
 * Update warmup configuration for a model
 */
router.put('/:id/warmup-config', async (req: any, res: any) => {
  try {
    const modelId = parseInt(req.params.id);
    ...
    const result = await db.query(updateQuery, [modelId, ...updateValues]);
