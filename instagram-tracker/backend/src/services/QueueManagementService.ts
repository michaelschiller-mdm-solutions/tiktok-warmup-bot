...
export class QueueManagementService {
   ...
   */
  async getAccountQueue(accountId: number): Promise<QueueItemDetailed[]> {
   ...
   */
  async getUpcomingContent(hours: number = 24): Promise<QueueItemDetailed[]> {
    const filters: QueueFilters = {
      status: 'queued',
   ...
   */
  async getOverdueContent(): Promise<QueueItemDetailed[]> {
    const filters: QueueFilters = {
      status: 'queued',
   ...
   */
  async checkQueueHealth(): Promise<HealthReport> {
  ...
  async detectBottlenecks(): Promise<Bottleneck[]> {
    ...
    // Failed items bottleneck
    if (stats.failed_count > 10) {
      bottlenecks.push({
        type: 'failed_items',
        severity: stats.failed_count > 30 ? 'high' : 'medium',
        affected_accounts: 0, // Will be calculated if needed
   ...
   */
  async generateHealthAlerts(): Promise<Alert[]> {
    const alerts: Alert[] = [];
    const overdueItems = await this.getOverdueContent();

    // Alert for overdue emergency content
  ...
  async getProcessingMetrics(): Promise<ProcessingMetrics> {
    ...
    return {
      ...
      avg_processing_time_ms: 5000, // Placeholder - would need more detailed tracking
  ...
  async getQueueSummary(): Promise<{
    stats: QueueStats;
    upcoming: QueueItemDetailed[];
    overdue: QueueItemDetailed[];
    recent_activity: QueueItemDetailed[];
  }> {
    const [stats, upcoming, overdue] = await Promise.all([
      this.getQueueStats(),
      this.getUpcomingContent(6), // Next 6 hours
      this.getOverdueContent()
    ]);

    // Get recent activity (last 10 posted/failed items)
    ...
    return {
      stats,
      upcoming: upcoming.slice(0, 20), // Limit to 20 items
      overdue: overdue.slice(0, 20), // Limit to 20 items
