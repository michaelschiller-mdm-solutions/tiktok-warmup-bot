...
export class ProxyAssignmentService {
  ...
  /**
   * Automatically assign a proxy to an account
   */
  async assignProxyToAccount(accountId: number): Promise<ProxyAssignmentResult> {
    try {
      // Check if account already has a proxy
      const existingProxy = await db.query(
        'SELECT proxy_id FROM accounts WHERE id = $1 AND proxy_id IS NOT NULL',
        [accountId]
      );
      ...
      // Find available proxy with capacity
      const availableProxy = await db.query(`
        SELECT 
          p.id,
          p.ip,
          p.port,
  ...
  /**
   * Assign proxies to multiple accounts in batch
   */
  async assignProxiesToAccounts(accountIds: number[]): Promise<ProxyAssignmentResult[]> {
    const results: ProxyAssignmentResult[] = [];
  ...
  /**
   * Get available proxies with capacity
   */
  ...
  /**
   * Get proxy assignment statistics
   */
  async getProxyAssignmentStats(): Promise<{
    totalProxies: number;
    activeProxies: number;
    fullyUtilizedProxies: number;
   ...
   */
  async unassignProxyFromAccount(accountId: number): Promise<ProxyAssignmentResult> {
    try {
      await db.query('BEGIN');
