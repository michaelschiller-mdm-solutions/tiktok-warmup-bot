...
import { ContentSelectionCriteria, ContentType, ContentAssignment, ContentCandidate } from '../types/warmupProcess';

export class ContentAssignmentService {

  /**
   * Select optimal content for a warmup phase
   */
  async selectContentForPhase(criteria: ContentSelectionCriteria): Promise<ContentCandidate | null> {
    try {
      let query = `
        SELECT 
      ...
      const params: any[] = [criteria.modelId, criteria.contentType];
      let paramIndex = 3;
      ...
      // Exclude recently used content if specified
      ...
      if (result.rows.length === 0) {
        // Fallback: try 'any' content type if specific type not found
        if (criteria.contentType !== ContentType.ANY) {
          const fallbackCriteria = { ...criteria, contentType: ContentType.ANY };
  ...
  /**
   * Select optimal text for a warmup phase
   */
  async selectTextForPhase(criteria: ContentSelectionCriteria): Promise<ContentCandidate | null> {
    try {
      let query = `
        SELECT 
  ...
  /**
   * Assign content to a warmup phase
   */
  async assignContentToPhase(
    ...
    try {
      ...
      // Select image content (for pfp, post, highlight, story)
      let selectedContent = null;
      let selectedText = null;

      if ([ContentType.PFP, ContentType.POST, ContentType.HIGHLIGHT, ContentType.STORY].includes(contentType)) {
        selectedContent = await this.selectContentForPhase(criteria);
      }

      // Select text content (for bio, post, highlight, story)
      if ([ContentType.BIO, ContentType.POST, ContentType.HIGHLIGHT, ContentType.STORY].includes(contentType)) {
      ...
      // Calculate assignment score based on content quality
      ...
      const assignmentResult = await db.query(assignmentQuery, assignmentParams);

      // Update warmup phase with content assignment
      await db.query(`
  ...
  /**
   * Get assigned content for a warmup phase
   */
  async getAssignedContent(warmupPhaseId: number): Promise<any> {
    try {
  ...
  /**
   * Mark content assignment as used with performance feedback
   */
  async markContentAsUsed(
    assignmentId: number,
    success: boolean,
    ...
    try {
      ...
      // The trigger will automatically update content usage statistics
    } catch (error) {
  ...
  /**
   * Get content assignment statistics
   */
  async getContentAssignmentStatistics(modelId: number, days: number = 30): Promise<any> {
    try {
      const result = await db.query(`
        SELECT 
          -- Assignment counts
   ...
   */
  async getContentPerformanceAnalytics(modelId: number): Promise<any> {
    try {
      const result = await db.query(`
        SELECT 
  ...
  /**
   * Blacklist underperforming content
   */
  async blacklistContent(contentId: number, contentCategory: 'image' | 'text', reason: string): Promise<void> {
    try {
   ...
   * Calculate assignment score based on content quality
   */
  private calculateAssignmentScore(imageContent?: ContentCandidate, textContent?: ContentCandidate): number {
    let totalScore = 0;
    let contentCount = 0;
  ...
  /**
   * Get assignment reason description
   */
  private getAssignmentReason(imageContent?: ContentCandidate, textContent?: ContentCandidate): string {
