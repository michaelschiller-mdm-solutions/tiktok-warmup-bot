...
export class MaintenanceTrackingService {
  ...
  /**
   * Get comprehensive maintenance status for all accounts
   */
  async getAllAccountsStatus(): Promise<MaintenanceStatus[]> {
    const query = `
    ...
    return result.rows.map(row => ({
      ...
      bot_status: 'idle' // Will be updated by bot activity monitor
    }));
  }

  /**
   * Get maintenance status for a specific account
   */
  async getAccountStatus(accountId: number): Promise<MaintenanceStatus | null> {
    const allStatuses = await this.getAllAccountsStatus();
    return allStatuses.find(status => status.account_id === accountId) || null;
  }

  /**
   * Get pending content queue items that need bot action
   */
  async getPendingContentQueue(limit: number = 50): Promise<ContentQueueStatus[]> {
    const query = `
      SELECT 
        cq.id,
        cq.account_id,
   ...
   */
  async markContentPosted(queueItemId: number, postedAt: Date = new Date()): Promise<void> {
    ...
    try {
      ...
      // Update sprint assignment progress if applicable
      ...
      // Update highlight maintenance if this was highlight content
   ...
   */
  async markContentFailed(queueItemId: number, errorMessage: string): Promise<void> {
    await db.query(`
      UPDATE content_queue 
      SET 
        status = 'failed',
  ...
  /**
   * Retry failed content (move back to queued with delay)
   */
  async retryFailedContent(queueItemId: number, delayMinutes: number = 30): Promise<void> {
   ...
   */
  async getOverdueContent(): Promise<ContentQueueStatus[]> {
    const query = `
      SELECT 
        cq.id,
        cq.account_id,
  ...
  /**
   * Update highlight maintenance timestamp
   */
  private async updateHighlightMaintenance(accountId: number, maintenanceTime: Date): Promise<void> {
    // Find the highlight group that received maintenance
    const query = `
    ...
    const result = await db.query(query, [maintenanceTime, accountId]);
   ...
   */
  async getMaintenanceAnalytics(days: number = 7): Promise<any> {
    const query = `
      WITH daily_stats AS (
        SELECT 
