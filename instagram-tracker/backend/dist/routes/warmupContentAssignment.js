...
router.post('/bulk-assign', async (req, res) => {
    try {
        const { lifecycle_states = ['ready', 'ready_for_bot_assignment'], force_reassign = false } = req.body;
    ...
    `;
        const accountsResult = await database_1.db.query(accountsQuery, [lifecycle_states]);
        ...
        for (const account of accounts) {
            ...
            `, [account.id, phaseName, phaseStatus]);
                ...
                const verifyResult = await database_1.db.query(`
          SELECT phase, status FROM account_warmup_phases 
          WHERE account_id = $1 
          ORDER BY phase
        `, [account.id]);
                console.log(`[Content Assignment] Account ${account.id} now has ${verifyResult.rows.length} phases:`, verifyResult.rows.map(r => `${r.phase}(${r.status})`).join(', '));
...
router.get('/status', async (req, res) => {
    try {
        const { lifecycle_states = ['ready', 'ready_for_bot_assignment', 'warmup'], page = 1, limit = 100 } = req.query;
      ...
      LIMIT $2 OFFSET $3
    `;
        const result = await database_1.db.query(query, [lifecycle_states, parseInt(limit), offset]);
        const countQuery = `
      SELECT COUNT(DISTINCT a.id) as total_count
      FROM accounts a
      WHERE a.lifecycle_state = ANY($1)
    `;
        const countResult = await database_1.db.query(countQuery, [lifecycle_states]);
...
router.post('/prepare-for-iphone/:accountId', async (req, res) => {
    try {
        const { accountId } = req.params;
        const { phase } = req.body;
        console.log(`📱 Preparing iPhone data for account ${accountId}, phase: ${phase || 'all'}`);
        const accountResult = await database_1.db.query('SELECT id, username, container_number FROM accounts WHERE id = $1', [accountId]);
    ...
    `;
        ...
        const phasesResult = await database_1.db.query(phaseQuery, params);
        ...
        const iphoneData = {
            ...
            phases: phases.map(phase => {
                ...
                if (phase.content_file_path) {
                    const fullPath = path_1.default.resolve(__dirname, '../../uploads', phase.content_file_path);
                    phaseData.content = {
...
router.post('/send-to-iphone/:accountId/:phase', async (req, res) => {
    try {
        const { accountId, phase } = req.params;
        const { iphone_ip = '192.168.178.65', iphone_port = 46952, use_nuclear_cleaner = true } = req.body;
    ...
    `, [accountId, phase]);
        ...
        if (phaseData.content_file_path) {
            try {
                const fullPath = path_1.default.resolve(__dirname, '../../uploads', phaseData.content_file_path);
...
router.post('/transition-to-warmup', async (req, res) => {
    try {
...
//# sourceMappingURL=warmupContentAssignment.js.map