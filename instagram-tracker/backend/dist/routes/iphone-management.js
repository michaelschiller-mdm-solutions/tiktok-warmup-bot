...
router.post('/', async (req, res) => {
    try {
        const { name, model, ip_address, port = 46952, ssh_user = 'mobile', ssh_password, xxtouch_port = 46952, notes } = req.body;
        ...
        if (ssh_password) {
            const passwordResult = await database_1.db.query('SELECT encrypt_iphone_ssh_password($1) as encrypted', [ssh_password]);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const iphoneResult = await database_1.db.query(`
    ...
    `, [id]);
        ...
        const containersResult = await database_1.db.query(`
      SELECT 
        ic.*,
    ...
    `, [id]);
        const logsResult = await database_1.db.query(`
      SELECT 
        ial.*,
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, model, ip_address, port, ssh_user, ssh_password, xxtouch_port, status, assigned_bot_id, container_creation_enabled, automation_enabled, notes } = req.body;
        const existingResult = await database_1.db.query('SELECT id FROM iphones WHERE id = $1', [id]);
      ...
      RETURNING id, name, model, ip_address, port, status
    `;
        const result = await database_1.db.query(updateQuery, [id, ...updateValues]);
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const existingResult = await database_1.db.query('SELECT name FROM iphones WHERE id = $1', [id]);
        if (existingResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'iPhone not found'
        ...
        const assignedContainers = await database_1.db.query(`
    ...
    `, [id]);
        ...
        await database_1.db.query('DELETE FROM iphones WHERE id = $1', [id]);
        res.json({
            success: true,
...
router.post('/assign-account', async (req, res) => {
    ...
    `, [accountId, iphoneId, containerNumber]);
        ...
        const containerResult = await database_1.db.query(`
      SELECT 
        ic.*,
...
router.post('/:id/sync-containers', async (req, res) => {
    ...
    try {
        ...
        const assignedResult = await client.query(`SELECT container_number FROM iphone_containers WHERE iphone_id = $1 AND assigned_account_id IS NOT NULL`, [id]);
        const assignedContainerNumbers = new Set(assignedResult.rows.map(r => r.container_number));
        const deleteResult = await client.query(`DELETE FROM iphone_containers WHERE iphone_id = $1 AND assigned_account_id IS NULL`, [id]);
        const newContainers = [];
...
//# sourceMappingURL=iphone-management.js.map