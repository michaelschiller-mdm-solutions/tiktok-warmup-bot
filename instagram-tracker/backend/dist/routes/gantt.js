...
const TimelineCalculationService_1 = require("../services/TimelineCalculationService");
const router = express_1.default.Router();
router.get('/timeline-data', async (req, res) => {
    try {
        const { start_date, end_date, container_width = 1200, zoom_level = 'week', pixels_per_day, major_tick_interval, minor_tick_interval, account_ids, sprint_ids } = req.query;
        ...
        const zoomConfig = {
            level: zoom_level,
            pixelsPerDay: pixels_per_day ? parseInt(pixels_per_day) : getDefaultPixelsPerDay(zoom_level),
            majorTickInterval: major_tick_interval ? parseInt(major_tick_interval) : getDefaultMajorTicks(zoom_level),
            minorTickInterval: minor_tick_interval ? parseInt(minor_tick_interval) : getDefaultMinorTicks(zoom_level),
        ...
        const timelineData = TimelineCalculationService_1.TimelineCalculationService.calculateTimelineData(mockAssignments, mockSprints, dateRange, parseInt(container_width), zoomConfig);
        res.json({
            success: true,
            data: {
...
function getDefaultPixelsPerDay(zoomLevel) {
    const defaults = {
        hour: 480,
        day: 200,
        week: 60,
...
function getDefaultMajorTicks(zoomLevel) {
    const defaults = {
        hour: 0.25,
        day: 1,
        week: 7,
...
function getDefaultMinorTicks(zoomLevel) {
    const defaults = {
        hour: 0.125,
        day: 0.5,
        week: 1,
