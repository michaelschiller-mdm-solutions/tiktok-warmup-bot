...
router.post('/:accountId/invalidate', async (req, res) => {
    try {
        const { accountId } = req.params;
        const changed_by = 'api_user';
        if (!accountId || isNaN(parseInt(accountId))) {
            return res.status(400).json({
                success: false,
                error: 'Invalid Account ID',
        ...
        const result = await AccountLifecycleService_1.AccountLifecycleService.invalidateAccount(parseInt(accountId), changed_by);
        if (result.success) {
            res.json({
                success: true,
...
router.post('/:accountId/complete-setup', async (req, res) => {
    try {
        const { accountId } = req.params;
