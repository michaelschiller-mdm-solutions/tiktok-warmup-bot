...
router.get('/models/:id/performance', async (req, res) => {
    try {
      ...
      WHERE model_id = $1;
    `;
        const followStatsResult = await database_1.db.query(followStatsQuery, [modelId]);
      ...
      WHERE model_id = $1;
    `;
        const accountStatsResult = await database_1.db.query(accountStatsQuery, [modelId]);
      ...
      LIMIT 90;
    `;
        const dailyActivityResult = await database_1.db.query(dailyActivityQuery, [modelId]);
        const daily_activity = dailyActivityResult.rows.map((row) => ({
