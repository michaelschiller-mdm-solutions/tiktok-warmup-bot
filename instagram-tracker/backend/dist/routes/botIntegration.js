...
router.post('/activity/start', async (req, res) => {
    try {
        ...
        const activityId = await botMonitor.recordActivityStart(bot_id, account_id || null, activity_type, metadata || {});
        res.json({
            success: true,
            data: {
...
router.post('/activity/complete', async (req, res) => {
    try {
        ...
        await botMonitor.recordActivityCompletion(activity_id, status, error_message || null, metadata || {});
        res.json({
            success: true,
            data: {
                activity_id,
...
router.post('/content/uploaded', async (req, res) => {
    try {
        ...
        if (bot_id) {
            const activityId = await botMonitor.recordActivityStart(bot_id, null, 'upload_content', { queue_item_id, instagram_post_id, ...metadata });
            await botMonitor.recordActivityCompletion(activityId, 'completed', null, { instagram_post_id, posted_at: postedAtDate });
        }
        res.json({
            success: true,
            data: {
...
router.post('/content/failed', async (req, res) => {
    try {
        ...
        if (bot_id) {
            const activityId = await botMonitor.recordActivityStart(bot_id, null, 'upload_content', { queue_item_id, ...metadata });
            await botMonitor.recordActivityCompletion(activityId, 'failed', error_message, { error_code, retry_delay_minutes });
        }
        res.json({
            success: true,
            data: {
...
router.post('/status/heartbeat', async (req, res) => {
    try {
        ...
        const activityId = await botMonitor.recordActivityStart(bot_id, null, 'health_check', {
            heartbeat: true,
            status,
            current_script,
            accounts_processing,
        ...
        await botMonitor.recordActivityCompletion(activityId, 'completed', null, { heartbeat_received: new Date() });
        res.json({
            success: true,
            data: {
                bot_id,
...
router.post('/script/started', async (req, res) => {
    try {
        ...
        const activityId = await botMonitor.recordActivityStart(bot_id, account_id || null, 'script_execution', { script_name, ...metadata });
        res.json({
            success: true,
            data: {
                activity_id: activityId,
...
router.post('/script/completed', async (req, res) => {
    try {
        ...
        await botMonitor.recordActivityCompletion(activity_id, status, error_message || null, { results, ...metadata });
        res.json({
            success: true,
            data: {
                activity_id,
...
//# sourceMappingURL=botIntegration.js.map