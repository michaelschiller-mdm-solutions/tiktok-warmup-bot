...
exports.ProxyProviderService = void 0;
const database_1 = require("../database");
const proxies_1 = require("../types/proxies");
class ProxyProviderService {
    async getProviders(status, includeStats = false) {
        try {
            let query = `
        SELECT 
      ...
      `;
            ...
            throw new proxies_1.ProxyError('Failed to fetch proxy providers', 'FETCH_ERROR', error);
        }
    }
    async getProviderById(id, includeStats = false) {
        try {
            let query = `
        SELECT 
          pp.id, pp.name, pp.monthly_cost_per_proxy, pp.contact_email,
      ...
      `;
            ...
            throw new proxies_1.ProxyError('Failed to fetch proxy provider', 'FETCH_ERROR', error);
        }
    }
    async createProvider(data) {
        try {
            if (!data.name || data.name.trim().length === 0) {
                throw new proxies_1.ProxyError('Provider name is required', 'VALIDATION_ERROR');
            }
            if (data.monthly_cost_per_proxy < 0) {
                throw new proxies_1.ProxyError('Monthly cost cannot be negative', 'VALIDATION_ERROR');
            }
            const query = `
      ...
      `;
            ...
            if (error.code === '23505' && error.constraint === 'proxy_providers_name_key') {
                throw new proxies_1.ProxyError('Provider name already exists', 'DUPLICATE_NAME');
            }
            if (error instanceof proxies_1.ProxyError) {
                throw error;
            }
            throw new proxies_1.ProxyError('Failed to create proxy provider', 'CREATE_ERROR', error);
        }
    }
    async updateProvider(id, data) {
        try {
            ...
            if (data.name !== undefined) {
                if (!data.name || data.name.trim().length === 0) {
                    throw new proxies_1.ProxyError('Provider name cannot be empty', 'VALIDATION_ERROR');
                }
            ...
            if (updateFields.length === 0) {
                throw new proxies_1.ProxyError('No fields to update', 'VALIDATION_ERROR');
            }
      ...
      `;
            ...
            throw new proxies_1.ProxyError('Failed to update proxy provider', 'UPDATE_ERROR', error);
        }
    }
    async deleteProvider(id) {
        try {
            const proxyCheckResult = await database_1.db.query('SELECT COUNT(*) as proxy_count FROM proxies WHERE provider_id = $1', [id]);
            const proxyCount = parseInt(proxyCheckResult.rows[0].proxy_count);
            if (proxyCount > 0) {
                throw new proxies_1.ProxyError(`Cannot delete provider with ${proxyCount} existing proxies. Remove all proxies first.`, 'HAS_PROXIES', { proxy_count: proxyCount });
            }
            const result = await database_1.db.query('DELETE FROM proxy_providers WHERE id = $1', [id]);
            ...
            throw new proxies_1.ProxyError('Failed to delete proxy provider', 'DELETE_ERROR', error);
        }
    }
    async getProviderProxies(providerId, page = 1, limit = 50) {
        try {
            const offset = (page - 1) * limit;
            const countResult = await database_1.db.query('SELECT COUNT(*) as total FROM proxies WHERE provider_id = $1', [providerId]);
            const total = parseInt(countResult.rows[0].total);
            const proxiesResult = await database_1.db.query(`
        SELECT 
      ...
      `, [providerId, limit, offset]);
            ...
            throw new proxies_1.ProxyError('Failed to fetch provider proxies', 'FETCH_ERROR', error);
        }
    }
}
exports.ProxyProviderService = ProxyProviderService;
//# sourceMappingURL=ProxyProviderService.js.map