...
exports.HighlightGroupService = void 0;
const database_1 = require("../database");
class HighlightGroupService {
    async createHighlightGroup(data) {
    ...
    async getHighlightGroups(filters = {}, page = 1, limit = 50) {
        let query = `
      SELECT 
        cs.*,
    ...
    `;
        ...
        const [result, countResult] = await Promise.all([
            database_1.db.query(query, params),
            database_1.db.query(countQuery, countParams)
        ]);
    ...
    async getHighlightGroupById(id) {
        const result = await database_1.db.query(`
      SELECT 
        cs.*,
        COUNT(sci.id) as content_count,
    ...
    async updateHighlightGroup(id, data) {
        const setClause = [];
        const params = [];
        let paramIndex = 1;
    ...
    async assignToAccounts(highlightGroupId, request) {
        const successfulAssignments = [];
        const failedAssignments = [];
        const groupResult = await database_1.db.query(`
    ...
    `, [highlightGroupId]);
        ...
        for (const accountId of request.account_ids) {
            try {
                const existingResult = await database_1.db.query(`
    ...
    async getAssignments(highlightGroupId) {
        const result = await database_1.db.query(`
      SELECT 
        ahg.*,
    ...
    async scheduleMaintenanceForGroup(highlightGroupId, request) {
        let query = `
    ...
    async updatePositions(accountId, request) {
    ...
    async getGroupAnalytics(highlightGroupId) {
    ...
    `, [highlightGroupId]);
        const positionResult = await database_1.db.query(`
    ...
    async getSystemMaintenanceOverview() {
        const overviewResult = await database_1.db.query(`
      SELECT 
      ...
      WHERE cs.is_highlight_group = true
    `);
        const todayMaintenanceResult = await database_1.db.query(`
      SELECT COUNT(*) as maintenance_today
      FROM content_queue cq
      WHERE cq.content_type = 'highlight'
        AND cq.created_at >= CURRENT_DATE
    `);
        const weekMaintenanceResult = await database_1.db.query(`
    ...
    async createContentBatch(highlightGroupId, request) {
        const result = await database_1.db.query(`
    ...
    async getContentBatches(highlightGroupId) {
        const result = await database_1.db.query(`
...
exports.HighlightGroupService = HighlightGroupService;
//# sourceMappingURL=HighlightGroupService.js.map