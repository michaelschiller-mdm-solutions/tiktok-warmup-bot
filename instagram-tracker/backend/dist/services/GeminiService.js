...
class GeminiService {
    ...
    static async loadSettings() {
        try {
            const result = await database_1.db.query('SELECT key, value FROM application_settings WHERE key IN ($1, $2)', ['gemini_api_key', 'gemini_model_name']);
    ...
    async callGemini(prompt, isValidation = false) {
        if (!this.config.apiKey) {
    ...
    loadAmericanSurnames() {
        try {
            const namelistPath = path_1.default.join(__dirname, '../../namelist.txt');
            const content = fs_1.default.readFileSync(namelistPath, 'utf-8');
    ...
    async generateUsernameVariations(originalUsername, count = 10) {
...
Erstelle insgesamt ${count} verschiedene Username-Variationen. Gib nur die Usernamen zurück, einen pro Zeile, ohne zusätzliche Erklärungen oder Nummerierungen.`;
    ...
    async generateBioVariations(originalBio, count = 10) {
...
Generate ${count} different bio variations, one per line:`;
    ...
    generateFallbackBioVariations(options) {
    ...
    generateFallbackUsernameVariations(username, count) {
        ...
        for (let i = 0; i < Math.min(count, surnames.length); i++) {
            ...
            variations.push(`${baseName}${surname}`, `${baseName}.${surname}`, `${baseName}_${surname}`, `${baseName}${surnameNoVowels}`, `${baseName}.${surnameNoVowels.toLowerCase()}`);
        }
        return Array.from(new Set(variations)).slice(0, count);
    }
}
